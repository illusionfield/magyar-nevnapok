...
const DEFAULT_OUTPUT_PATH = path.join(process.cwd(), "output", "unified-primary-report.json");
...
function readSingleJsonFromZip(zipPath) {
  ...
  if (eocdOffset === -1) {
    throw new Error(`EOCD not found in zip: ${zipPath}`);
  }
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (sig !== 0x02014b50) {
      throw new Error(`Invalid central directory header at ${ptr}`);
    }
  ...
  if (!jsonEntry) {
    throw new Error(`No JSON file found in zip: ${zipPath}`);
  }

  const localSig = buffer.readUInt32LE(jsonEntry.localHeaderOffset);

  if (localSig !== 0x04034b50) {
    throw new Error(`Invalid local header for ${jsonEntry.fileName}`);
  }
  ...
  if (jsonEntry.compressionMethod === 0) {
    ...
    throw new Error(`Unsupported zip compression method: ${jsonEntry.compressionMethod}`);
  }
