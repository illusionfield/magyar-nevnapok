...
import { normalizeNameForMatch } from "./lib/primary-registry.js";

const WIKIPEDIA_CATEGORY_URL = "https://hu.wikipedia.org/wiki/Kategória:Az_év_napjai";
const DEFAULT_OUTPUT_PATH = path.join(process.cwd(), "output", "wiki-primary-registry.json");
...
async function main() {
  console.log("Starting Wikipedia nameday scrape.");
  ...
  try {
    ...
    console.log(
      `Discovered ${discoveredDays.length} Wikipedia day page(s), scraping ${selectedDays.length} page(s) with concurrency ${concurrency}.`
    );
    ...
    console.log(`Saved ${days.length} Wikipedia primary day(s) to ${outputPath}`);
  } finally {
    await browser.close();
  }
}
...
async function discoverDayPages(browser) {
  ...
  try {
    ...
    if (!selection.links.length) {
      throw new Error("No day links were found on the Wikipedia category page.");
    }
    ...
    console.log(`Using category selector: ${selection.selector}`);
    console.log(`Discovered ${days.length} distinct Wikipedia day link(s).`);
...
async function retryScrapeDay(browser, page, dayMeta) {
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      lastError = error;
      console.warn(`Attempt ${attempt}/${retries} failed for ${dayMeta.monthDay}: ${error.message}`);
...
async function scrapeDay(page, dayMeta) {
  ...
  if (names.length === 0) {
    console.warn(`No Wikipedia nameday names found for ${dayMeta.monthDay} (${dayMeta.url}).`);
  }
...
async function safeClosePage(page) {
  ...
  try {
    ...
    // Ignore already-closed page errors.
  }
}
