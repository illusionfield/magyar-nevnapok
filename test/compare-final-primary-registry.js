...
const DEFAULT_NORMALIZED_REGISTRY_PATH = path.join(
  process.cwd(),
  "output",
  "unified-primary-report.json"
);
const DEFAULT_INPUT_PATH = path.join(process.cwd(), "output", "nevnapok.json");
const DEFAULT_REPORT_PATH = path.join(
  process.cwd(),
  "output",
  "final-primary-registry-report.json"
);
const MONTH_NAMES = [
  "Január",
  "Február",
...
function buildRawDayMap(payload) {
  if (!Array.isArray(payload?.names)) {
    throw new Error("A névnap JSON nem tartalmaz érvényes names tömböt.");
  }
...
function printReport(report) {
  printKeyValueTable(
    ...
    [
      ...
      ["Névnap JSON", report.inputs.inputPath],
