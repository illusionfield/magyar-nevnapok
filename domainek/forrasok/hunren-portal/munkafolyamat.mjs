...
const DEFAULT_OUTPUT_PATH = kanonikusUtvonalak.adatbazis.nevnapok;
...
async function main() {
  ...
  const [primaryRegistry, legacyPrimaryRegistry] = await Promise.all([
    loadPrimaryRegistryOrThrow(primaryRegistryPath, "nevnapok pipeline futtat vegso-primer-feloldas"),
    loadPrimaryRegistryOrThrow(
      legacyPrimaryRegistryPath,
      "nevnapok pipeline futtat legacy-primer-epites"
    ),
  ]);
  ...
  try {
    ...
    console.log(
      `Összesen ${sortedDiscoveredNames.length} név található, ezek közül ${selectedNames.length} részletes oldal kerül feldolgozásra ${concurrency} párhuzamos feldolgozóval.`
    );
...
async function discoverNames(browser) {
  ...
  for (const index of indexes) {
    ...
    try {
      ...
      const nemCimke = index.gender === "female" ? "női" : "férfi";
      console.log(`${pageNames.length} ${nemCimke} név került felderítésre innen: ${index.url}`);
    } finally {
      await page.close();
    }
  }
...
async function retryScrapeName(browser, page, nameMeta) {
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      lastError = error;
      console.warn(`${attempt}/${retries}. kísérlet sikertelen ennél a névnél: ${nameMeta.name}. ${error.message}`);
...
async function loadPrimaryRegistryOrThrow(filePath, buildCommand) {
  try {
    ...
    if (error?.code === "ENOENT") {
      throw new Error(`A primerjegyzék nem található itt: ${filePath}. Futtasd ezt: ${buildCommand}`);
    }
